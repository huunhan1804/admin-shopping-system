...
const Sidebar = () => {
  ...
  useEffect(() => {
    loadPendingCounts();
    const interval = setInterval(loadPendingCounts, 300000);
  ...
  useEffect(() => {
    ...
    if (path.startsWith("/products")) {
      ...
      setExpandedMenus((prev) => ({ ...prev, insurance: true }));
  ...
  const Badge = ({ count, variant = "yellow" }) => {
  ...
  const menuItems = [
    ...
    },
    {
      ...
      icon: Package,
      expandable: true,
      description: "Quản lý catalog product",
      children: [
        {
          path: "/products",
  ...
  return (
    <>
      ...
      <aside
        className={`
        ${sidebarWidth} bg-gradient-to-b from-gray-800 via-gray-800 to-gray-900 text-white 
      ...
      `}
      >
        {/* Header - Enhanced collapsed version */}
        <div
        ...
        >
          {isCollapsed ? (
            /* Collapsed state - Nút trượt ra với hiệu ứng đẹp */
            <div className="flex justify-center">
              <button
                onClick={toggleCollapse}
                className="relative p-3 rounded-xl bg-white bg-opacity-10 hover:bg-opacity-20 transition-all duration-300 group shadow-lg hover:shadow-xl hover:scale-105"
            ...
            /* Expanded state - Logo và nút thu gọn */
            <div className="flex items-center justify-between">
              ...
              <button
                onClick={toggleCollapse}
                className="hidden lg:block p-2 rounded-lg hover:bg-white hover:bg-opacity-20 transition-all duration-300 group"
