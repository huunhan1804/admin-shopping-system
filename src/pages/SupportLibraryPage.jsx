...
const SupportLibraryPage = () => {
  ...
  const handleDeleteCategory = async (categoryId) => {
    if (
      !window.confirm(
        "Bạn có chắc muốn xóa danh mục này? Tất cả bài viết trong danh mục cũng sẽ bị xóa."
      )
    )
      return;
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        // cập nhật ngay UI
        setCategories((prev) =>
          prev.filter((c) => c.supportCategoryId !== categoryId)
        );
  ...
  const handleSaveCategory = async (data) => {
    try {
      ...
      if (categoryModal.mode === "create") {
        ...
        response = await supportService.updateCategory(
          categoryModal.data.supportCategoryId,
          data
        );
        if (response.status === 200) {
  ...
  const handleSaveArticle = async (data) => {
    try {
      ...
      if (articleModal.mode === "create") {
        response = await supportService.createArticle(
          articleModal.categoryId,
          data
        );
        if (response.status === 200)
          showToast("success", "Thêm bài viết thành công");
      } else {
        response = await supportService.updateArticle(
          articleModal.data.articleId,
          data
        );
        if (response.status === 200)
          showToast("success", "Cập nhật bài viết thành công");
      }
      await loadArticles(selectedCategory.supportCategoryId);
      setArticleModal({
        show: false,
        mode: "create",
        categoryId: null,
          ...
          {selectedCategory ? (
            <>
              ...
              <ArticleList
                ...
                deletingArticleId={deletingArticleId} // <-- thêm
                togglingArticleId={togglingArticleId} // <-- thêm
              />
            </>
          ) : (
            <div className="bg-white rounded-lg shadow-sm p-12 text-center">
              ...
              <p className="text-gray-600">
                Vui lòng chọn một danh mục từ danh sách bên trái để xem và quản
                lý các bài viết
              </p>
            </div>
          )}
